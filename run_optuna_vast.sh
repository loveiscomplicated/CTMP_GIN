...
# -----------------------
if [[ $# -lt 2 ]]; then
  echo "Usage: bash run_optuna_vast.sh <model_name> <config_path>"
  echo "Example: bash run_optuna_vast.sh gin configs/gin.yaml"
  exit 1
fi
...
BOT_NAME="vast_optuna_${MODEL_NAME}"
...
tmux set-environment -g CONTAINER_API_KEY "${CONTAINER_API_KEY:-}"
tmux set-environment -g VAST_INSTANCE_ID "${VAST_INSTANCE_ID:-}"
